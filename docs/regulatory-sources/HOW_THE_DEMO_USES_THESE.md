...
- **Annex 5b Project Finance E&S Screening Questionnaire (148 IFC PS-aligned Yes/No items)** → `lib/regulatory/esdd/annex5b-pf-questions.ts` (every question tagged `ifcPS: "PS1".."PS8"`), scoring in `lib/regulatory/esdd/annex5b-pf-scoring.ts`, types in `lib/regulatory/esdd/annex5b-pf-types.ts`; wizard at `components/bfi/pf-screening/wizard.tsx` served from `app/pf-screening/[loanId]/page.tsx`; API in `app/api/pf-screening/**`; DB in `scripts/supabase-pf-screening.sql`
